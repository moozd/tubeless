...
func (r *Renderer) PrepareFrame(scr *screen.Screen, cfg config.Config, cellW, cellH float32, scrollOffset int, sel Selection) {
	// The content-scroll glide's row/column bounds come from diffing the
	// live screen (see screen.DetectContentShift's callers in
	// cmd/tubeless), so they only mean what they say when scrollOffset is
	// 0 — otherwise BuildInstances would apply them to whatever unrelated
	// scrollback rows currently occupy those row numbers in the view.
	// Suppressing here (rather than only at detection time) also covers
	// the case where the user scrolls back mid-glide, before it's fully
	// eased out.

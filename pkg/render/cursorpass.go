...
// Draw renders the cursor at cell (col, row) — in grid coordinates, i.e.
// before the centering offset — into dst, which covers the whole window.
// offsetX/Y are the cell grid's centering offset in pixels (same convention
// as CellPass.Draw). cellW/cellH are the physical pixel size of one cell,
// cfg supplies the cursor color and edge softness, and bright is the
// breathing-phase intensity (0 = hidden).
func (c *CursorPass) Draw(dst *FBO, col, row, offsetX, offsetY, cellW, cellH float32, outW, outH int, bright float32, cfg config.Config) {
	...
	gl.Uniform2f(u("uPos"), offsetX+col*cellW, offsetY+row*cellH)
	gl.Uniform2f(u("uSize"), cellW, cellH)
